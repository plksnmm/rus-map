...
модератора.
# Таблицы административной аутентификации

- `app.admin_users`: нормализованное уникальное имя, Argon2id-хеш и признак активности;
- `app.admin_sessions`: только SHA-256-хеши session/CSRF-токенов, срок действия, время последнего использования и отзыва;
- `app.admin_login_throttles`: постоянные счётчики неудачных входов по односторонним ключам.

Миграция `bcf6d33f74a2` создаёт эти таблицы после `7b82e019c4a6`. Открытый текст пароля и session-токена в базе отсутствует.

# Аудит модерации заявок

Миграция `a16b9d4e2f73` добавляет nullable-внешний ключ
`app.place_submissions.moderated_by_admin_id` на `app.admin_users`. Новые
approve/reject-решения всегда записывают администратора и `moderated_at`.
Nullable сохраняет совместимость со старыми финальными записями, которые могли
появиться до административной аутентификации; pending-заявка обязана иметь оба
поля пустыми.

Миграция `c42f7d18e6a1` добавляет nullable-поле
`app.place_submissions.address` длиной до 500 символов. Адрес хранится как
пользовательская подсказка для модератора; географическим источником истины
остаются `latitude` и `longitude`.
