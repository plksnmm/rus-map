...
# Очередь предложений мест

Ревизия `7b82e019c4a6` добавляет `app.place_submissions`. Координаты предложения
хранятся отдельными числами до модерации; PostGIS Point создаётся только при
одобрении через обычный `PlaceRepository`.

Ограничения базы обеспечивают:

- диапазоны широты `-90..90` и долготы `-180..180`;
- непустой заголовок;
- статусы `pending`, `approved`, `rejected`;
- отсутствие `approved_place_id` и времени решения у ожидающей заявки;
- обязательную связь с одним `Place` у одобренной заявки;
- отсутствие публичного места у отклонённой заявки;
- уникальность `approved_place_id` и индекс очереди по статусу и времени.

Ссылки на источники хранятся в JSONB после Pydantic-проверки HTTP(S), но backend
по ним не обращается. Проверка содержания источника остаётся действием
модератора.
