...
total и пустой результат без запуска базы.
...
откатывает транзакцию. Тесты не выполняют автоматический downgrade, чтобы
случайно не удалить данные из локальной базы разработчика.
